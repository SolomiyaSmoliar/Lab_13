...
class LinkedBST(AbstractCollection):
    ...
    def is_balanced(self):
        ...
        :return:
        '''
        return self.height() < ceil(2 * log(len(self) + 1, 2) - 1)

    def range_find(self, low, high):
        '''
        Returns a list of the data in the tree, where low <= item <= high."""
        :param low:
        :param high:
        :return:
        '''
        return list(filter(lambda i: i >= low and i <= high, self.inorder()))
    ...
    def rebalance(self):
        ...
        def recurse(data):
            if not data:
                return None
            middle = len(data) // 2
            return BSTNode(data[middle], left=recurse(data[:middle]),\
                 right=recurse(data[middle + 1:]))
        self._root = recurse(data)
    ...
    def successor(self, item):
        ...
        :rtype:
        """
        large_items = list(filter(lambda i: i > item, self))
        if len(large_items) > 0:
            return min(large_items)
        else:
            return None
    ...
    def predecessor(self, item):
        ...
        :rtype:
        """
        small_items = list(filter(lambda i: i < item, self))
        if len(small_items) > 0:
            return max(small_items)
        else:
            return None
...
#ас пошуку 10000 випадкових слів у словнику, який представлений у вигляді бінарного дерева пошуку. Бінарне дерево пошуку будується на основі послідовного додавання в дерево слів зі словника, який впорядкований за абеткою.
